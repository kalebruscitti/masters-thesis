	...
	\end{equation}
	It is the dimension of $\mathcal{H}$ which we hope to compute. If $B_s$ is the set of all Bohr-Sommerfeld points, and for each $b\in B_s$ $S_b$ is the space of global covariant constant sections of $\LL|_{\pi^{-1}(b)}$, then Sniatycki (cite) proves that when there is a natural isomorphism:
	\begin{equation}
...
\subsection{Moduli of Connections on a Trinion}
	...
	\end{theorem}
	Thus, the fibres corresponding to interior triples, which are the generic fibres, are tori of dimension $3g-3$. It is the non-generic fibres, who have holonomy angles on the boundary, which remain to be understood.
	...
	\subsection{something about P here?}
